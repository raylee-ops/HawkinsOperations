...
```powershell
$sigmaYml  = (Get-ChildItem -Recurse -Filter *.yml -Path ".\detection-rules\sigma" -ErrorAction SilentlyContinue).Count
$sigmaYaml = (Get-ChildItem -Recurse -Filter *.yaml -Path ".\detection-rules\sigma" -ErrorAction SilentlyContinue).Count
...
$playbooks = (Get-ChildItem -Recurse -Filter IR-*.md -Path ".\incident-response\playbooks" -ErrorAction SilentlyContinue).Count

Write-Host "Sigma (.yml/.yaml): $sigma"
